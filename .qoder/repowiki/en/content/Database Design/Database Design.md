Now I have all the necessary information to update the documentation. Based on my analysis, I can see that:

1. A new migration file `V6__add_conference_type.sql` was added that adds a `type` column to the conferences table
2. The `Conference` entity now has a `type` field with `ConferenceType` enum values
3. The migration includes proper indexing for type-based queries and descriptive comments
4. The existing `ConferenceRepository` queries still use string literals for status but don't yet use the new type field
...
## Update Summary
**Changes Made**
- Added new conference type column to conferences table with default value of 'SCHEDULED'
- Implemented proper indexing for type-based queries with filtered index on deleted_at
- Added descriptive comments for the new conference type column
- Updated Conference entity with ConferenceType enum containing SCHEDULED and PERMANENT values
- Enhanced conference lifecycle management with type distinction between scheduled and permanent rooms
...
R_Conf["ConferenceRepository.java"]
end
V1 --> E_Tenant
V1 --> E_User
V1 --> E_Conf
...
- [Conference.java:1-245](file://jmp-domain/src/main/java/com/jmp/domain/entity/Conference.java#L1-L245)
...
- Conferences
  - Purpose: Jitsi conference rooms with scheduling, features, and metadata. **Updated**: Now distinguishes between scheduled conferences (with fixed time) and permanent rooms (always available).
  - Key fields: id (UUID PK), room_name, display_name, description, tenant_id (FK), created_by (FK), status, **type**, scheduling timestamps, actual timestamps, recurring flags, password hash, feature toggles, jitsi_options/metadata (JSONB), timestamps, soft-delete deleted_at.
  - Constraints: Unique constraint on room_name+tenant_id (filtered by deleted_at); FKs to tenants and users; **type** field with default value 'SCHEDULED'.
  - Indexes: tenant_id, status, created_by, scheduled range, room_name+tenant_id (filtered), **type** (filtered by deleted_at).
  - Lifecycle: Status includes SCHEDULED, ACTIVE, ENDED, CANCELLED; **type** distinguishes between SCHEDULED and PERMANENT conference types; soft-delete sets deleted_at and status.
  - **New**: ConferenceType enum with SCHEDULED (fixed-time conferences) and PERMANENT (always-available rooms) values.
...
- Conferences
  ...
  - **Updated**: ConferenceType enum with SCHEDULED (fixed-time conferences) and PERMANENT (always-available rooms) values.
  - **New**: Proper indexing for type-based queries with filtered index on deleted_at.
- Conference Participants
...
- Indexes (filtered where applicable):
  ...
  - Conferences: tenant_id, status, created_by, scheduled_start/scheduled_end, room_name+tenant_id (filtered), **type (filtered by deleted_at)**.
...
- [V6__add_conference_type.sql:8-11](file://jmp-web/src/main/resources/db/migration/V6__add_conference_type.sql#L8-L11)
...
- Business Rules:
  ...
  - **Updated**: Conference type distinction between SCHEDULED (fixed-time) and PERMANENT (always-available) rooms.
  - **New**: Type-based filtering and querying capabilities for different conference categories.
...
- Typed Repositories:
  ...
  - **Updated**: ConferenceRepository provides enhanced querying capabilities with conference type filtering.
- Fetch Strategies:
...
- Projection and Filtering:
  ...
  - **New**: Conference type-based filtering through ConferenceType enum values.
...
- **New**: Conference type distinction affects lifecycle management - PERMANENT conferences have different scheduling and availability rules than SCHEDULED conferences.
...
- Additional Tables:
  ...
  - **New**: V6 adds conference type column with default value 'SCHEDULED' and proper indexing.
- Version Control:
...
- **New**: Conference type-based access control - different permissions and restrictions for SCHEDULED vs PERMANENT conference types.
...
- Indexes:
  ...
  - **New**: Type-based filtering with filtered index on conferences.type improves performance for conference type queries.
...
- **New**: Conference type queries benefit from dedicated index on type column with soft-delete filtering.
...
- Common Issues:
  ...
  - **New**: Conference type filtering issues: Verify conference type enum values and proper indexing.
- Diagnostics:
  ...
  - **New**: Check conference type index usage for type-based filtering queries.
...
**Updated**: Recent enhancements include improved Jitsi domain configuration for local development environments, enhanced password security through the adoption of bcrypt $2b$12$ algorithm, and the introduction of conference type distinction between SCHEDULED and PERMANENT rooms. The new conference type column with proper indexing enables efficient type-based filtering and supports different conference lifecycle management strategies.
